...
def diffusion_reaction_solver(v: np.ndarray, xmax: float = 1.0, tmax: float = 1.0, D: float = 0.01, k: float = 0.01, Nx: int = 101, Nt: int = 101) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    Shapes:
        ...
        u(x, t): (Nx, Nt) 
    """
    _, _, u = solve_ADR(xmin = 0, xmax = xmax, tmin = 0, tmax = tmax, 
    ...
    xt = np.asarray(np.meshgrid(np.linspace(0, xmax, Nx), np.linspace(0, tmax, Nt))).T
